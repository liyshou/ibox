...
/*unsigned char* AESEncryptEx(const unsigned char* aInput,
		   ...
		   int* aOutputLength){
			   int         i = 0,j = 0,times = 0,zero_addition = 0,addition=0;
			   uint8       input[16],output[16],*cipher;
			   aes_context ctx;
			   
			   zero_addition=16-aInputLength%16;
			   if(zero_addition==16)
				   zero_addition=0;
			   
			   if(!zero_addition){
				    times = aInputLength/16;
			   }
			   else{

                    times = aInputLength/16+1;
			   }

			   cipher = (uint8*) malloc (sizeof(uint8)*16*(times)+1);

			   aes_set_key( &ctx, aSessionKey, aSessionKeyLength*8);

			   for(i=0;i<times;i++){
				   if(i<(times-1)||!(zero_addition)){
					   memcpy(input ,aInput+16*i ,16);
				   }
				   else{
					   addition = 0x30+zero_addition;
					   memcpy(input ,aInput+16*i ,aInputLength%16);
					    for(j=aInputLength%16;j<16;j++)
					//	   input[j] = 0;
						input[j]=addition;
				   }
               aes_encrypt(&ctx ,input ,output);
			   memcpy(cipher+16*i , output , 16);

			   }

			 //  cipher[16*i] = (unsigned char)zero_addition;
			   //*aOutputLength=16*(times)+1;
				*aOutputLength=16*times;
			   return cipher;
...
unsigned char* AESDecryptEx(const unsigned char* aInput,
		  ...
		  int* aOutputLength){
			   int         i = 0,j = 0,times = 0;
			   uint8       input[16],output[16],*plain;
			   aes_context ctx;
			   times = aInputLength/16;
			   plain = (uint8*) malloc (sizeof(uint8)*16*(times)+1);
			   memset(plain,0,16*(times)+1);
...
//{
//    int out=0,out1=0;
//	unsigned char *plain=(unsigned char*)"china-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isichina-isi",*cipher;
//	
//	unsigned char key[16]="123456789";
//	cipher=PKIAESEncryptEx(plain,32,key,16,&out);
//	plain=PKIESDecryptEx(cipher,out,key,16,&out1);
//
//
//    return( 0 );
//}
